...
// ------------------------------------------------------------ console_new ---
console_t *console_new(float font_size) {
  ...
  normal.font = texture_font_new_from_file(self->atlas, font_size, f);

  markup_t bold = normal;
  bold.bold = 1;
  bold.font = texture_font_new_from_file(self->atlas, font_size, f);
  ...
  bold.bold = 1;
  italic.italic = 1;
  italic.font = texture_font_new_from_file(self->atlas, font_size, f);
  ...
  error.background_color.r = 1.00;
  error.background_color.g = 1.00;
  error.background_color.b = 1.00;
  ...
  self->markup[MARKUP_NORMAL] = error;
...
// -------------------------------------------------------- console_delete ---
void console_delete(console_t *self) {
...
// ----------------------------------------------------- console_add_glyph ---
void console_add_glyph(console_t *self, char *current, char *previous,
                       markup_t *markup) {
...
// -------------------------------------------------------- console_render ---
void console_render(console_t *self) {
  char *cur_char;
  char *prev_char;
...
// ------------------------------------------------------- console_connect ---
void console_connect(console_t *self, const char *signal,
...
// --------------------------------------------------------- console_print ---
void console_print(console_t *self, char *text) {
...
// ------------------------------------------------------- console_process ---
void console_process(console_t *self, const char *action,
                     const unsigned char key) {
  size_t len = strlen(self->input);

  if (strcmp(action, "type") == 0) {
    if (len < MAX_LINE_LENGTH) {
      memmove(self->input + self->cursor + 1, self->input + self->cursor,
    ...
    if (strcmp(action, "enter") == 0) {
      ...
      memmove(self->input + self->cursor, self->input + self->cursor + 1,
      ...
      if ((len + l) < MAX_LINE_LENGTH) {
        memmove(self->input + self->cursor + l, self->input + self->cursor,
                (len - self->cursor) * sizeof(char));
        memcpy(self->input + self->cursor, self->killring, l * sizeof(char));
        self->cursor += l;
      }
...
void init(float font_size) {
  ...
  shader = shader_load(
      "/Users/SeungheonOh/Documents/Terminal/src/shaders/v3f-t2f-c4f.vert",
      "/Users/SeungheonOh/Documents/Terminal/src/shaders/v3f-t2f-c4f.frag");
